...
# Complete combinations if 0 cases
count_allage <- count_allage_n |>
  ungroup() |>
  complete(cohort, nesting(gender, disease_number_new), fill=list(n=0))

# Join N and n for 'all'into one table 
count_allage = merge(x = count_allage_N, y = count_allage [
