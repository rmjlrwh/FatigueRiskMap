# -------------------------------------------------------------------
###### Combine patients, diagnoses - reference group (registered patietns) MALES #####
...
# Add cohort tag
n_diags_ref_f <- n_diags_ref %>%
  mutate(cohort = 2)

# Save results
save(n_diags_ref_f, file = "n_diags_ref_f.RData")
...
n_diags_ref_f_broad <- n_diags_ref_f |>
  select(!c("prev_diag_2yr", "prevdiag_period", "months_diff"))

rm("n_diags_ref_f")

# Merge broad disease group num
n_diags_ref_f_broad = merge(x = n_diags_ref_f_broad, y = lookup_diseases_meta [
  , c("disease_number_new", "broad_disease_group_num")]
  , by =c("disease_number_new"), all.x = TRUE)

n_diags_ref_f_broad <- n_diags_ref_f_broad |>
...
save(n_diags_ref_f_broad, file = "n_diags_ref_f_broad.RData")

rm(n_diags_ref_f_broad)
